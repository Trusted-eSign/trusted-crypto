		...
		for (int i = 0, c = sizeof(listProviders) / sizeof(*listProviders); i < c; i++){
			...
			for (int i = 0; i < listPkiObj.size(); i++){
				...
				if (strcmp(itemTemp->type->c_str(), "CERTIFICATE") == 0){
					...
					itemTemp->certSignatureDigestAlgorithm = new std::string(listPkiObj[i]["SignatureDigestAlgorithm"].asString());
					itemTemp->certPublicKeyAlgorithm = new std::string(listPkiObj[i]["PublicKeyAlgorithm"].asString());
				}
					...
					itemTemp->crlAuthorityKeyid = new std::string(listPkiObj[i]["AuthorityKeyid"].asString());
					itemTemp->crlCrlNumber = new std::string(listPkiObj[i]["CrlNumber"].asString());
				}
...
void CashJson::importJson(Handle<PkiItem> item){
	...
	try{		
		...
		if (strcmp(item->type->c_str(), "CERTIFICATE") == 0){
			...
			jsnBuf["SignatureDigestAlgorithm"] = item->certSignatureDigestAlgorithm->c_str();
			jsnBuf["PublicKeyAlgorithm"] = item->certPublicKeyAlgorithm->c_str();
		}
			...
			jsnBuf["AuthorityKeyid"] = item->crlAuthorityKeyid->c_str();
			jsnBuf["CrlNumber"] = item->crlCrlNumber->c_str();
		}
