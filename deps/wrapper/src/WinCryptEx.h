...
#define CRYPT_UTF8                              0x08000000 // container name in UTF8 encoding

#define ACQUIRE_CONTEXT_SUPPORTED_FLAGS		(CRYPT_GENERAL | CRYPT_NOSERIALIZE | CRYPT_REBOOT | CRYPT_PROMT_INSERT_MEDIA | CRYPT_UECDATACONTEXT | CRYPT_CMS_HIGHLOAD_NOSERIALIZE | CRYPT_NO_CONTAINER_CACHE | CRYPT_LOCAL_PASSWORD_CACHE | CRYPT_UTF8)
...
#define CRYPT_PIN_WND_PARAM_BOTH    (CRYPT_PIN_WND_PARAM_INFO | CRYPT_PIN_WND_PARAM_AUTH )/*!< Тип передаваемой информации.  Передается указатель на служебную информацию, переданную диалогу ввода пароля и информация об аутентификации. Установка переданного указателя 
	...
	*/
	typedef struct _CRYPT_NK_INFO_PARAM {
		...
		char parts[1]; /*! Последовательность n строк (если сделан вызов с флагом CRYPT_UTF8, то строки в кодировке UTF8,
					   *  иначе в кодировке CP_ACP). */
	} CRYPT_NK_INFO_PARAM;
...
#define CSP_INFO_SIZE sizeof(CSP_INFO)
	...
	*/
	typedef union _CRYPT_PIN_INFO_SOURCE {
		...
		char encryption[1]; /*! Строка имени контейнера зашифрования
							*  (если сделан вызов с флагом CRYPT_UTF8, то строки в кодировке UTF8, иначе в кодировке CP_ACP) */
