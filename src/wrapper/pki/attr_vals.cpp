...
AttributeValueCollection::AttributeValueCollection(Attribute *data) {
	LOGGER_FN();
	...
	if (this->set_ == NULL) {
		//sk_ASN1_TYPE_new_null(this->set_); изменено на строку ниже
...
void AttributeValueCollection::push(void *val) {
	...
	ASN1_TYPE_set1(_type, this->data_->asnType(), val);
...
std::string AttributeValueCollection::items(int index) {
	LOGGER_FN();
	...
	std::string res((char *) out, len);
	//delete out;
	return res; //ASN1_TYPE *_dupType = 
}
...
void AttributeValueCollection::set(int index, void *val){
	...
	LOGGER_OPENSSL(ASN1_TYPE_new);
	ASN1_TYPE *_type = ASN1_TYPE_new();
	/*if (!ASN1_TYPE_set1(_type, this->data_->asnType(), val)){
		THROW_EXCEPTION(0, AttributeValueCollection, NULL, "ASN1_TYPE_set1");
	}*/
